...
function shallowCopy(obj) {
  if (Array.isArray(obj)) {
    return obj.map((elem) => shallowCopy(elem));
  }
  if (obj instanceof Object) {
    const temp = {};
    Object.keys(obj).forEach((key) => {
      temp[key] = shallowCopy(obj[key]);
    });
    return temp;
  }
  return obj;
 ...
 * @return {Object} - The object with the specified keys removed
 *
 * @example
...
function removeProperties(obj, keys) {
  const keySet = Array.isArray(keys) ? new Set(keys) : new Set([keys]);
  const result = {};
  Object.keys(obj).forEach((key) => {
    if (!keySet.has(key)) {
      result[key] = obj[key];
...
function makeWord(lettersObject) {
  const word = [];
  // Iterate over each key-value pair in the lettersObject
  Object.keys(lettersObject).forEach((letter) => {
    if (Object.prototype.hasOwnProperty.call(lettersObject, letter)) {
      // Retrieve the positions array for the current letter
      const positions = lettersObject[letter];
      // Append the letter to the word for each position
...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    if (bill === 25) {
      ...
      if (count25 === 0) {
        res = false; // Seller doesn't have enough change
      }
      count25 -= 1;
      count50 += 1;
    } else if (bill === 100) {
...
function sortCitiesArray(arr) {
  arr.sort((a, b) => {
    ...
    // If country names are equal, compare city names
